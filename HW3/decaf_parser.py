...
def p_method_decl(p):
    ...
    if (len(p) == 8):
        ...
        p[0] = Method(p[1],p[2],p[3],None,p[6])
...
def p_constructor_decl(p):
    ...
    if(len(p)==7):
        ...
        p[0] = Constructor(p[1],p[2],None,p[5])
...
def p_stmt(p):
    ...
    if(p[1]=='if'):
        if(len(p)==8):
            Statement.if_stmt(stmt, p[3], p[5], p[7])
        else:
            Statement.if_stmt(stmt, p[3], p[5], None)
    elif(p[1]=='while'):
        Statement.while_stmt(stmt, p[3], p[5])
    elif(p[1]=='for'):
        if(len(p)==10):
            Statement.for_stmt(stmt, p[3], p[5], p[7], p[9])
        elif(len(p)==9):
            if(p[3]==';'):
                Statement.for_stmt(stmt, None, p[4], p[6], p[8])
            elif(p[6]==';'):
                Statement.for_stmt(stmt, p[3], p[5], None, p[8])
            else:
                Statement.for_stmt(stmt, p[3], None, p[6], p[8])
        elif(len(p)==8):
            if(p[3]!=';'):
                Statement.for_stmt(stmt, p[3], None, None, p[7])
            elif(p[4]!=';'):
                Statement.for_stmt(stmt, None, p[4], None, p[7])
            else:
                Statement.for_stmt(stmt, None, None, p[5], p[7])
        else:
            Statement.for_stmt(stmt, None, None, None, p[6])
    elif(p[1]=='return'):
        if(len(p)==4):
            Statement.return_stmt(stmt, p[2])
        else:
            Statement.return_stmt(stmt, None)
    elif(len(p)==3):
        if(p[1]=='break'):
            Statement.break_stmt(stmt)
        elif(p[1]=='continue'):
            Statement.continue_stmt(stmt)
        else:
            Statement.expr_stmt(stmt, p[1])
    elif(p[1]==';'):
        Statement.skip_stmt(stmt)
    else:
        p[0] = Statement.block_stmt(stmt, p[1])
    p[0] = stmt
...
def p_literal_int(p):
    'literal : INTCONST'
    literal=Expression()
    Expression.constant_expression(literal, 'Integer-constant', p[1])
    p[0]=literal
def p_literal_float(p):
    'literal : FLOATCONST'
    literal=Expression()
    Expression.constant_expression(literal, 'Float-constant', p[1])
    p[0]=literal
def p_literal_string(p):
    'literal : STRINGCONST'
    literal=Expression()
    Expression.constant_expression(literal, 'String-constant', p[1])
    p[0]=literal
def p_literal(p):
    '''literal : NULL
               | TRUE
    ...
    literal=Expression()
    if(p[1]=='null'):
        Expression.constant_expression(literal, 'None-constant', p[1])   
    else:
        Expression.constant_expression(literal, 'Boolean-constant', p[1]) 
    p[0]=literal
...
def p_primary(p):
    ...
    if(p[1]=='new'):
        expr=Expression()
        if(len(p)==6):
            Expression.new_object_expression(expr, p[2], p[4])
        else:
            Expression.new_object_expression(expr, p[2], None)
        p[0]=expr
    elif(p[1]=='this'):
        expr=Expression()
        Expression.this_expression(expr)
        p[0]=expr
    elif(p[1]=='super'):
        expr=Expression()
        Expression.super_expression(expr)
        p[0]=expr
    elif(len(p)==2):
        p[0]=p[1]
    else:
...
def p_field_access(p):
    ...
    expr=Expression()
    if(len(p)==4):
        Expression.field_access_expression(expr, p[1], p[3])
    else:
        Expression.field_access_expression(expr, None, p[1])
    p[0]=expr
...
def p_method_invocation(p):
    ...
    expr=Expression()
    if(len(p)==5):
        Expression.method_call_expression(expr, p[1], p[3])
    else:
        Expression.method_call_expression(expr, p[1], None)
    p[0]=expr
...
def p_expr(p):
    ...
    if(p[1]=='-' or p[1]=='+' or p[1]=='!'):
        expr=Expression()
        Expression.unary_expression(expr, p[1], p[2])
        p[0]=expr
    elif(len(p)==4):
        expr=Expression()
        Expression.binary_expression(expr, p[1], p[2], p[3])
        p[0]=expr
    else:
        p[0]=p[1]
...
def p_assign(p):
    ...
    expr=Expression()
    if(len(p)==4):
        Expression.assign_expression(expr, p[1], p[3])
    elif(p[1]=='++'):
        Expression.auto_expression(expr, p[2], 'auto-increment', 'pre')
    elif(p[1]=='--'):
        Expression.auto_expression(expr, p[2], 'auto-decrement', 'pre')
    elif(p[2]=='++'):
        Expression.auto_expression(expr, p[1], 'auto-increment', 'post')
    else:
        Expression.auto_expression(expr, p[1], 'auto-decrement', 'post')
    p[0]=expr
