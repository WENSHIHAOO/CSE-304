...
class Class(object):
    def __init__(self, ID, EXTENDS_ID, class_body_decls, line):
        self.register=''
        self.line=line
        self.class_name=ID
        ...
        for class_body_decl in class_body_decls:
            if type(class_body_decl) is Constructor:
                if(class_body_decl.constructor_name != self.class_name):
                    print()
                    print("Constructor Error: Constructor and Class have different names.")
                    print("Class Name: '%s', at line %d" %(self.class_name, self.line))
                    print("Constructor: at line %d" %(class_body_decl.line))
                    print()
                    sys.exit()
                ...
                if(method_name in self.methods):
                    ...
                    print("Class Name: '%s', at line %d" %(self.class_name, self.line))
                    print("Method Name: '%s', at line %d" %(class_body_decl.method_name, class_body_decl.line))
                    print()
                    sys.exit()
                class_body_decl.containing_class=ID
                self.methods[method_name]=class_body_decl
                self.var_method[class_body_decl.method_name]=Variable(method_name, class_body_decl.line)
                ...
                for var in class_body_decl.field_name:
                    if(var+ID in self.fields_check):
                        ...
                        print("Class Name: '%s', at line %d" %(self.class_name, self.line))
                        print("Fields Name: '%s', at line %d" %(var, class_body_decl.line))
                        print()
                        sys.exit()
                    self.fields_check[var+ID]=var
                self.fields.append(class_body_decl)
                self.var_field[var]=Variable(var, class_body_decl.line)
        global class_table
        ...
        global var_class
        var_class[ID]=Variable(ID, class_body_decl.line)
        
    def __str__(self):
        global var_array_ref
...
class Constructor(object):
    def __init__(self, modifier, ID, formals, block, line):
        self.register=''
        self.line=line
                ...
                for var in stmt.variable_table:
                    if(var.variable_name in self.constructor_parameters
                       or var.variable_name in self.variable_table):
                        ...
                        print("Constructor Name: '%s', at line %d" %(ID, self.line))
                        print("Variable Name: '%s', at line %d" %(var.variable_name, var.line))
                        print()
                        sys.exit()
...
class Method(object):
    def __init__(self, modifier, type_, ID, formals, block, line):
        self.register=''
        self.line=line
        self.method_name=ID
        global method_id
                ...
                for var in stmt.variable_table:
                    if(var.variable_name in self.method_parameters
                       or var.variable_name in self.variable_table):
                        ...
                        print("Method Name: '%s', at line %d" %(self.method_name, self.line))
                        print("Variable Name: '%s', at line %d" %(var.variable_name, var.line))
                        print()
                        sys.exit()
                    var.variable_id=variable_id
...
class Field(object):
    def __init__(self, modifier, var_decl):
        self.register=''
        self.line=var_decl[0].type.line
        self.field_name=[]
        variable_id=1
...
class Variable(object):
    ...
    def var_type(type_, variables, line):
...
class Type(object):
    ...
    def __str__(self):
        if(self.type in ['int', 'boolean', 'float', 'string', 'void', 'error', 'null']):
            print(self.type, end='')
        else:
...
class Statement(object):
    def __init__(self):
        ...
        self.expr=None
        self.check_var={}
    def if_stmt(self, expr, if_stmt, else_stmt, line):
        self.line=line
        self.id='if'
        self.condition=expr
        self.then_stmt=if_stmt
        self.stmt=else_stmt
    def while_stmt(self, expr, while_stmt, line):
        self.line=line
        self.id='while'
        self.condition=expr
        self.stmt=while_stmt
    def for_stmt(self, stmt_expr, expr, update_stmt_expr, stmt, line):
        self.line=line
        self.id='for'
        ...
        self.stmt=stmt
    def return_stmt(self, expr, line):
        self.line=line
        self.id='return'
        self.expr=expr
    def expr_stmt(self, stmt_expr, line):
        self.line=line
        self.id='expr'
        self.expr=stmt_expr
    def block_stmt(self, block, line):
        self.line=line
        self.id='block'
        self.stmt=block
    def break_stmt(self, line):
        self.line=line
        self.id='break'
    def continue_stmt(self, line):
        self.line=line
        self.id='continue'
    def skip_stmt(self, line):
        self.line=line
        self.id='skip'
    def check_var_array(self, vars):
        for var in vars.variable_table:
            if(var.variable_name in self.check_var):
                ...
                print("Scope Name: '%s', at line %d" %(self.id, self.line))
                print("Variable Name: '%s', at line %d" %(var.variable_name, var.line))
                print()
                sys.exit()
...
class Expression(object):
    ...
    def assign_expression(self, lhs, expr, line):
        self.line=line
        self.id='assign'
        self.expression1=lhs
        self.expression2=expr
    def auto_expression(self, lhs, increment_decrement, post_pre, line):
        self.line=line
        self.id='auto'
        self.expression1=lhs
        self.operator1=increment_decrement
        self.operator2=post_pre
    def field_access_expression(self, base, field, line):
        self.line=line
        self.id='field_access'
        self.expression1=base
        self.expression2=field
    def method_call_expression(self, field_access, arguments, line):
        self.line=line
        self.id='method_call'
        self.expression1=field_access
        self.arguments=arguments
    def new_object_expression(self, base, arguments, line):
        self.line=line
        self.id='new_object'
        self.expression1=base
        self.arguments=arguments
    def this_expression(self, line): 
        self.line=line
        self.id='this'
        self.expression2='this'
    def super_expression(self, line): 
        self.line=line
        self.id='super'
        self.expression2='super'
        ...
        if(self.id=='constant'):
            ...
            if(self.expression1==None):
                ...
                if error==0:
                    ...
                    print("Variable Name: '%s', at line %d" %(self.expression2, self.line))
                    print()
                    sys.exit()
            else:
                if type(self.expression1) is str:
                    ...
                    if(self.field_id==0):
                        print(" , localORmethod )",end='')
                    else:
                        print(' ,',self.field_id,")",end='')
                else:
            ...
            print("New_object(",self.expression1,',',self.constructor_id,', [',end='')
            if(type(self.arguments) is list):
                for i, a in enumerate(self.arguments):
                    if i == len(self.arguments) - 1:
                        a.__str__(var_array_ref)
...
def Initialization():
    ...
    scan_int=Method(['public', 'static'], type_int, 'scan_int', None, [], 0)
    scan_float=Method(['public', 'static'], type_float, 'scan_float', None, [], 0)
    Class('In', None, [scan_int, scan_float], 0)
    
    vari=Variable('i', 0)
    vari.type=type_int
    printi=Method(['public', 'static'], 'void', 'print', [vari], [], 0)
    
    varf=Variable('f', 0)
    varf.type=type_float
    printf=Method(['public', 'static'], 'void', 'print', [varf], [], 0)
    
    varb=Variable('b', 0)
    varb.type=type_boolean
    printb=Method(['public', 'static'], 'void', 'print', [varb], [], 0)
    
    vars=Variable('s', 0)
    vars.type=type_string
    prints=Method(['public', 'static'], 'void', 'print', [vars], [], 0)
    Class('Out', None, [printi, printf, printb, prints], 0)